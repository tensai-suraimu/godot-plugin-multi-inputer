...
@export_range(-1, 7) var joypad_id := -1:
	get:
		return joypad_id
	set(id):
		if id == joypad_id:
...
## 线速度缩放倍率
@export var velocity_scale := Vector2.ONE
...
func create_robot(character: CharacterBody2D, commands: Array[String] = []) -> MInput.Robot:
	if _robot:
		_robot.start()
		_robot.queue(commands)
		return _robot
	# 延迟加载，避免用不到机器人的场景产生额外开销
	_robot = MInput.Robot.new(character)
	_robot.start()
	_robot.queue(commands)
	character.add_child(_robot)
	return _robot

## 关闭并移除机器人，停止模拟输入并释放机器人。
...
func handle_move(character: CharacterBody2D, delta: float) -> void:
	...
	character.velocity.x = speed_x * velocity_scale.x
...
func _handle_jump(character: CharacterBody2D, delta: float, direction: float) -> void:
		...
		if not on_floor:
			...
			if dive_speed and just_jump:
				...
				if character.velocity.y < dive_speed:
					character.velocity.y = dive_speed * velocity_scale.y
			_apply_gravity(character, delta)
			return
		_jump_combo = 0 # 重置连跳次数
		# 接触地面，尝试穿过单向平台
	...
	if not just_jump:
		...
		if wall_glide and character.velocity.y >= wall_glide and on_wall:
			_jumping = JumpStatus.FALL
			character.velocity.y = wall_glide * velocity_scale.y
			return
		# 浮空滑翔
		if glide_speed and character.velocity.y >= glide_speed:
			_jumping = JumpStatus.STAY
			character.velocity.y = glide_speed * velocity_scale.y
			return
	...
	if on_floor:
		...
		_jump_combo = 0 # 重置连跳次数
		character.velocity.y = -jump_speeds[0] * velocity_scale.y
		if direction:
			_direction = direction
		return
	...
	if direction and on_wall and wall_speed:
		...
		if normal.x and (normal.x < 0) != (direction < 0):
			...
			character.velocity.y = -wall_speed * velocity_scale.y
	...
	if _jump_combo < jump_speeds.size():
		_jumping = JumpStatus.JUMP
		character.velocity.y = -jump_speeds[_jump_combo] * velocity_scale.y
		if direction:
			_direction = direction
		return
...
func _ma(action_name: String) -> String:
	if _robot and _robot.is_running():
